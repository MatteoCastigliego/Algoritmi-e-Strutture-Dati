...
Item leggi_da_tastiera();
Item leggi_da_file(FILE *f);
...
int main()
{
    ...
    while (1)
    {
        ...
        if (scelta == 'T')
        {
            persona = leggi_da_tastiera();
            head = inserisci_in_lista(persona, head);
        }

        if (scelta == 'F')
        {
            persona = leggi_da_file(fin);
...
Item leggi_da_file(FILE *f)
{
    ...
    char codice[5], nome[MAX_LEN], cognome[MAX_LEN], data_nascita[MAX_LEN], via[MAX_LEN], citta[MAX_LEN];
    int CAP;
    if (fscanf(f, "%s %s %s %s %s %s %d", codice, nome, cognome, data_nascita, via, citta, &CAP) == 7)
    {
        persona.CAP = CAP;
        persona.citta = strdup(citta);
        persona.codice = strdup(codice);

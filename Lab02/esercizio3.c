...
void elabora_dati(char *nome_file, dizionario *tratte);


int main(void)
{
    dizionario *tratte;
    char filename[MAX_LEN_STR], scelta[1];

    do{
        printf("Inserire il nome del file: ");
        scanf("%s", filename);
        elabora_dati(filename, tratte);

        printf("Vuoi inserire dati da un altro file (Y/N): ");
        scanf("%s", scelta);

    }while(strcmp(scelta, "Y") == 0 || strcmp(scelta, "Y") == 0);
